...
export default function ModelCard({
  ...
  const getRecommendationBadge = () => {
    ...
    return (
      <View className={`flex flex-row items-center gap-1 px-2 py-1 rounded ${tierColors[recommendation.deviceTier] || 'bg-blue-500'}`}>
        <TierIcon size={10} color="#fff" weight="bold" />
        <Text className="text-white text-xs font-medium capitalize">{recommendation.deviceTier}</Text>
      </View>
    );
  };
  ...
  const getQualityIndicator = () => {
    ...
    return (
      <View className="flex flex-row items-center gap-1">
        <Text className={`${color} text-xs font-mono`}>{recommendation.qualityScore}</Text>
  ...
  const getQuantizationBadge = () => {
    if (!recommendation?.quantization || recommendation.quantization === 'Unknown') return null;
    
    return (
      <View className="flex flex-row items-center gap-1 px-2 py-0.5 bg-white/10 rounded">
        <Text className="text-white/80 text-xs font-mono">{recommendation.quantization}</Text>
      </View>
    );
  };
      ...
      <View className="flex flex-row gap-x-4 items-center justify-between">
        ...
        <View className="flex flex-col gap-y-1 flex-1">
          ...
          {recommendation?.isRecommended && (
            <Text className="text-emerald-400 text-xs italic" numberOfLines={1} ellipsizeMode="tail">
