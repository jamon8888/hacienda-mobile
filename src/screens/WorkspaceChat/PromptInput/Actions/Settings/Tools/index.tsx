...
export default function ToolsActionSheet() {
  ...
  const loadTools = async () => {
    ...
    for (const tool of ToolsManager.configurableTools)
      updates[tool.id] = tool.defaultEnabled;
    const storedToolSettings: Record<string, boolean> =
      await uiStore.getFromStorage("tools", {});
    for (const [tool, enabled] of Object.entries(storedToolSettings))
      updates[tool] = enabled;
    ...
    <BottomSheetModal
      ...
      <ScrollView
        ...
        <View
          ...
          {ToolsManager.configurableTools
            ...
            .map(tool => (
              <TogglableItem
                ...
                isOn={toolSettings[tool.id]}
                onToggle={handleToggle.bind(null, tool.id)}
              />
            ))}
          <View
            style={{ gap: 12 }}
            ...
            {ToolsManager.configurableTools
              .filter(tool => tool.category === "appConnections")
              .map(tool => (
                <TogglableItem
                  key={tool.id}
                  title={tool.name}
  ...
  return (
    <View className="flex w-full flex-row items-center justify-between">
      <View className="flex flex-col items-start justify-between">
        <Text
          style={{ color: !primary && !isOn ? "#9F9FA0" : "#FFF" }}
          className={`text-[14px] font-semibold`}>
          {title}
        </Text>
        <Text style={{ color: "#9F9FA0", maxWidth: "90%" }} className="text-sm">
          {description}
        </Text>
      </View>
