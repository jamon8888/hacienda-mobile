...
const MODEL_OPTIONS = [
    ...
    { value: 'small', label: 'Small', size: '200 MB', description: 'Best accuracy' },
];
...
export default function TranscriptionOptionsSheet({ onConfirm }: TranscriptionOptionsProps) {
    ...
    return (
        ...
        >
            <View style={{ paddingHorizontal: 30, paddingBottom: insets.bottom, paddingTop: 20 }}>
                <View className='flex flex-row items-center gap-2 mb-4'>
                    <MusicNotes size={20} color="#6CE9A6" />
                    <Text className='text-white text-xl font-medium'>Audio Transcription</Text>
                </View>

                <Text style={{ color: '#9F9FA0' }} className='text-sm uppercase mb-2'>Model Size</Text>
                <View className='mb-4'>
                    {MODEL_OPTIONS.map((model) => (
                        <TouchableOpacity
                            ...
                            style={{
                                ...
                                borderColor: selectedModel === model.value ? '#3B82F6' : 'transparent',
                            }}
                            className='flex flex-row items-center justify-between'
                        >
                            <View className='flex-1'>
                                <Text className='text-white text-base'>{model.label}</Text>
                                <Text style={{ color: '#9F9FA0' }} className='text-xs'>{model.description} | {model.size}</Text>
                            </View>
                            <Radio size={20} color={selectedModel === model.value ? '#3B82F6' : '#9F9FA0'} weight="fill" />
                        </TouchableOpacity>
                    ))}
                </View>

                <Text style={{ color: '#9F9FA0' }} className='text-sm uppercase mb-2'>Language</Text>
                <View className='flex flex-row flex-wrap gap-2 mb-6'>
                    {LANGUAGE_OPTIONS.map((lang) => (
                        <TouchableOpacity
                            ...
                            style={{
                                backgroundColor: selectedLanguage === lang.value ? '#3B82F6' : '#27282A',
                                paddingHorizontal: 12,
                                paddingVertical: 6,
                                borderRadius: 16,
                            }}
                        >
                            <Text style={{ color: selectedLanguage === lang.value ? '#FFF' : '#9F9FA0' }} className='text-sm'>
                                {lang.label}
                            </Text>
                        </TouchableOpacity>
                    ))}
                ...
                >
                    <Text className='text-white text-base font-medium'>Transcribe & Import</Text>
                </TouchableOpacity>
            </View>
        </BottomSheetModal>
    );
