...
export default function CitationsActionSheet() {
  ...
  const [focusedCitations, setFocusedCitations] = useState<
    WorkspaceChatType["response"]["citations"]
  >([]);
  ...
  return (
    <BottomSheetModal
      ...
      <ScrollView
        style={{ paddingHorizontal: 30, paddingBottom: insets.bottom }}>
        <View
  ...
  return (
    <View
      ...
      <View className="flex flex-row items-center justify-between w-full">
        ...
        {citation.document.score && (
          <Text style={{ color: "#9F9FA0" }}>
            Score: {numberToPercentageString(citation.document.score)}
          </Text>
        )}
      </View>
...
function WebSearchCitation({
  citation,
}: {
  citation: IAgentWebSearchCitation;
}) {
  return (
    <TouchableOpacity
      onPress={() => Linking.openURL(citation.reference.url)}
      className="flex flex-col items-start justify-between w-full"
      style={{ gap: 12 }}>
      <View className="flex flex-col" style={{ gap: 2 }}>
        <View className="flex flex-row items-center justify-between w-full">
          <View
            ...
            style={{ gap: 4, maxWidth: "95%" }}>
            <Globe size={18} color="#7cd4fd" />
            <Text
              className="text-white text-lg font-semibold"
              numberOfLines={1}
              ellipsizeMode="tail">
              {citation.reference.title || getOrigin(citation.reference.url)}
            </Text>
            <ArrowSquareOut size={18} color="#888" />
          </View>
        </View>
        <Text style={{ color: "#9F9FA0", opacity: 0.7 }} className="text-xs">
