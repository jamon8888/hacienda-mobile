...
const smartMessages = {
  ...
  research: {
    ...
    onClick: {
      before: async function () {
        ...
        await uiStore.setToStorage("tools", {
          ...enabledTools,
          webSearch: true,
        } as never);
      },
  ...
  calendar: {
    text: function () {
      ...
      if (isWeekend || (isPast5PM && tomorrowIdx > 6))
        return "What is on my calendar for Monday?";
...
export default function EmptyList({ height }: { height: number }) {
  ...
  async function getRandomMessages(limit = 3) {
    ...
    for (let i = 0; i < Object.keys(smartMessages).length; i++) {
      ...
      const message =
        availableMessages[randomKey as keyof typeof availableMessages];
      const text =
  ...
  return (
    <View
      ...
      {messages.map((message, index) => (
        <DefaultMessage key={index} item={message as never} />
      ))}
    </View>
  );
}
...
export function EmptyListLoading({ height }: { height: number }) {
  return (
    <View
      ...
      <Text style={{ color: "#888" }} className="text-center">
...
function DefaultMessage({
  ...
  item: { text: string; onClick: { before: () => void; after: () => void } };
}) {
  ...
  function onPress() {
    ...
    const listener = uiStore.emitter.addListener(
      CHAT_HANDLER_EVENTS.ASSISTANT_RESPONSE_COMPLETE,
      () => {
        console.log(
          "Assistant response complete - running default message after hook.",
        );
        item.onClick.after();
        listener.remove();
      },
  ...
  return (
    <TouchableOpacity
      onPress={onPress}
      style={{
        width: screenDimensions.width / 1.6,
        paddingVertical: 10,
        paddingHorizontal: 8,
      }}
