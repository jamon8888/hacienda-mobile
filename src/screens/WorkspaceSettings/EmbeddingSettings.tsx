...
import { ArrowLeft, Globe, CaretDown, Check, Warning, Info } from "phosphor-react-native";
import { useNavigation } from "@react-navigation/native";
import { PATHS } from "@/utils/paths";
import uiStore from "@/store/UIStore";
import { useEffect, useState } from "react";
import { MULTILINGUAL_EMBEDDING_MODELS, MultilingualEmbeddingModelId, DEFAULT_MULTILINGUAL_EMBEDDING_MODEL } from "@/utils/models/defaults";
...
const ENGINE_OPTIONS: { value: MultilingualEmbeddingModelId; label: string; description: string; size: string; quality: number; dimensions: number }[] = [
    {
...
export function EmbeddingSettingsView({ workspace, goToPage }: EmbeddingSettingsViewProps) {
    const navigation = useNavigation();
    const insets = useSafeAreaInsets();
    const [config, setConfig] = useState(workspace.embeddingConfig || {
    ...
    const selectedEngine = ENGINE_OPTIONS.find(e => e.value === config.engine) || ENGINE_OPTIONS[0];
    const supportsMatryoshka = selectedEngine.value === 'nomic-embed-text-v2-moe';
    const availableDimensions = supportsMatryoshka ? DIMENSION_OPTIONS : DIMENSION_OPTIONS.filter(d => d.value <= selectedEngine.dimensions);
    ...
    async function saveConfig() {
        ...
        await workspace.update({ field: 'slug', value: workspace.slug }, { embeddingConfig: newConfig });
        setConfig(newConfig);
    ...
    return (
        ...
        >
            ...
            <View style={{ paddingHorizontal: 30, paddingTop: insets.top, paddingBottom: 20 }} className="w-full flex flex-row items-center justify-center relative">
                ...
                <Text style={{ maxWidth: '80%' }} numberOfLines={1} ellipsizeMode="middle" className="text-white text-lg font-medium">Embedding Settings</Text>
            </View>
            ...
            <ScrollView showsVerticalScrollIndicator={false} contentContainerClassName="flex flex-col" contentContainerStyle={{ paddingHorizontal: 18, paddingBottom: 100, gap: 24, backgroundColor: '#1B1B1E' }}>
                ...
                <View className="w-full flex flex-col" style={{ gap: 12 }}>
                    <View className="flex flex-row items-center justify-between">
                        <Text style={{ color: '#9F9FA0' }} className="text-sm uppercase">Embedding Engine</Text>
                    ...
                    <Text style={{ color: '#9F9FA0' }} className="text-xs">
                        {selectedEngine.description} | {selectedEngine.size} | French MTEB: {selectedEngine.quality}
                    </Text>

                    {/* Engine Picker Modal */}
                    {showEnginePicker && (
                        <View className="absolute inset-0 bg-black/50 flex items-center justify-center z-10" style={{ top: insets.top }}>
                            <View className="bg-gray-900 rounded-xl p-6 w-full mx-4 max-h-[80%]">
                                <Text className="text-white text-xl font-bold mb-4">Select Embedding Engine</Text>
                                <ScrollView>
                                    {ENGINE_OPTIONS.map(engine => (
                                        <TouchableOpacity
                                            ...
                                            style={{
                                                ...
                                                borderColor: config.engine === engine.value ? '#3B82F6' : 'transparent',
                                            }}
                                        ...
                                        >
                                            <View className="flex-1">
                                                <View className="flex flex-row items-center gap-2 mb-2">
                                                    <Text className="text-white text-lg">{engine.label}</Text>
                                                    {config.engine === engine.value && <Check size={20} color="#3B82F6" />}
                                                </View>
                                                <Text className="text-gray-400 text-sm">{engine.description}</Text>
                ...
                {supportsMatryoshka && (
                    <View className="w-full flex flex-col" style={{ gap: 12 }}>
                        <View className="flex flex-row items-center justify-between">
                            <Text style={{ color: '#9F9FA0' }} className="text-sm uppercase">Embedding Dimensions</Text>
                            <Info size={18} color="#6F6F71" />
                        </View>
                        <TouchableOpacity
                            style={{ backgroundColor: '#27282A', padding: 14, gap: 20 }}
                        ...
                        <Text style={{ color: '#9F9FA0' }} className="text-xs">
                            Matryoshka truncation: smaller dimensions = faster search, less storage. 256 dims retains ~99% quality.
                        </Text>
                        ...
                        {showDimensionPicker && (
                            <View className="absolute inset-0 bg-black/50 flex items-center justify-center z-10" style={{ top: insets.top }}>
                                <View className="bg-gray-900 rounded-xl p-6 w-full mx-4 max-h-[80%]">
                                    <Text className="text-white text-xl font-bold mb-4">Select Dimensions</Text>
                                    <ScrollView>
                                        {availableDimensions.map(dim => (
                                            <TouchableOpacity
                                                ...
                                                style={{
                                                    ...
                                                    borderColor: config.dimensions === dim.value ? '#3B82F6' : 'transparent',
                                                }}
                                                onPress={() => handleDimensionChange(dim.value)}
                                                className="flex flex-row items-center justify-between"
                                            >
                                                <Text className="text-white text-lg">{dim.label}</Text>
                                                {config.dimensions === dim.value && <Check size={20} color="#3B82F6" />}
                                            </TouchableOpacity>
                                        ))}
                                    </ScrollView>
                                    <TouchableOpacity
                ...
                {!supportsMatryoshka && (
                    <View className="w-full flex flex-col" style={{ gap: 12 }}>
                        ...
                        <View style={{ backgroundColor: '#27282A', padding: 14, gap: 20 }} className="w-full flex flex-row items-center rounded-lg">
                            <View className="flex flex-row gap-2 items-center">
                                <Globe size={18} color="#FFF" />
                                <Text className="text-white text-lg">Dimensions</Text>
                            </View>
                        ...
                        <Text style={{ color: '#9F9FA0' }} className="text-xs">
                            This model has fixed dimensions. Switch to Nomic v2 MoE for Matryoshka truncation.
                        </Text>
                    </View>
                )}
                ...
                <View className="w-full flex flex-col" style={{ gap: 12 }}>
                    <Text style={{ color: '#9F9FA0' }} className="text-sm uppercase">Auto-detect Language</Text>
                    <TouchableOpacity
                        style={{ backgroundColor: '#27282A', padding: 14, gap: 20 }}
                        className="w-full flex flex-row items-center rounded-lg"
                    ...
                    <Text style={{ color: '#9F9FA0' }} className="text-xs">
                        Automatically detect document language and route to optimal embedder. When disabled, uses selected engine for all documents.
                    </Text>
                </View>

                {/* Re-embed Workspace */}
                <View className="w-full flex flex-col" style={{ gap: 12 }}>
                    <Text style={{ color: '#9F9FA0' }} className="text-sm uppercase">Re-embed Workspace</Text>
                    <TouchableOpacity
                        style={{ backgroundColor: 'rgba(59,130,246,0.2)', padding: 14, gap: 20 }}
                        className="w-full flex flex-row items-center rounded-lg"
                    ...
                    <Text style={{ color: '#9F9FA0' }} className="text-xs">
                        Re-process all documents with current embedding settings. This will delete existing vectors and create new ones.
                    </Text>
                    ...
                    {showReembedConfirm && (
                        <View className="absolute inset-0 bg-black/50 flex items-center justify-center z-10" style={{ top: insets.top }}>
                            <View className="bg-gray-900 rounded-xl p-6 w-full mx-4">
                                <Text className="text-white text-xl font-bold mb-4">Re-embed Workspace?</Text>
                                <Text className="text-gray-300 mb-6">
                                    This will re-process all documents in this workspace using the current embedding engine and dimensions.
                                    Existing vectors will be replaced. This operation cannot be undone and may take several minutes.
                                </Text>
                ...
                {/* Current Model Info */}
                <View className="w-full flex flex-col" style={{ gap: 12 }}>
                    <Text style={{ color: '#9F9FA0' }} className="text-sm uppercase">Current Configuration</Text>
                    <View style={{ backgroundColor: '#27282A', padding: 14, gap: 12 }} className="w-full rounded-lg">
                        ...
                        <View className="flex flex-row justify-between">
                            ...
                            <Text className="text-white">{config.autoDetectLanguage ? 'Enabled' : 'Disabled'}</Text>
                        </View>
                        <View className="flex flex-row justify-between">
                            <Text className="text-gray-400">Matryoshka</Text>
                            <Text className="text-white">{supportsMatryoshka ? 'Supported' : 'Not supported'}</Text>
                        </View>
                    </View>
                </View>
            </ScrollView>
