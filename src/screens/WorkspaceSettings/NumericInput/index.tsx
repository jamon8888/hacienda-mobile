...
export function NumericInputView({
  workspace,
  goToPage,
  field,
  title,
  ...
  const [value, setValue] = useState<number>(
    (workspace[field] as unknown as number) ?? resetValue,
  );
        ...
        {suggestions?.length > 0 && (
          <View style={{ gap: 10, marginTop: 30 }} className="flex flex-col">
            ...
            <View
              ...
              {suggestions
                .filter(
                  suggestion => suggestion.toString() !== value.toString(),
                )
                .map(suggestion => (
                  <TouchableOpacity
                    key={suggestion}
                    style={{ paddingHorizontal: 10, paddingVertical: 5 }}
                    onPress={() => handleValueChange(suggestion)}
                    className="flex flex-row items-center justify-center bg-white/10 rounded-full">
