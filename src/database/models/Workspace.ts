...
export type WorkspaceDBType = Model &
  WorkspaceType & {
    threads: {
...
export default class Workspace extends Model {
  ...
  static writableFields = {
    ...
    systemPrompt: {
      validate: (value: string) => {
        ...
        if (value.length > Workspace.maxSystemPromptLength)
          error = `System prompt must be less than ${Workspace.maxSystemPromptLength} characters long`;
        return { valid: !error, error };
      },
    },
    temperature: {
    ...
    contextLength: {
      validate: (value: number) => {
        ...
        if (typeof value !== "number" || isNaN(numValue))
          error = "Context length must be a number";
    ...
    embeddingConfig: {
      validate: (value: any) => {
        ...
        if (
          value.engine &&
          !MULTILINGUAL_EMBEDDING_MODELS[
            value.engine as MultilingualEmbeddingModelId
          ]
        ) {
    ...
    return {
      ...
      remoteModelTag: async (): Promise<string> => {
        ...
        try {
          ...
          const response = await external.sendCommand("model-tag", {
            workspaceSlug: remoteConfig.slug,
          });
  ...
  static async find(
    ...
    if (withThreads) {
      const workspacesWithThreads = await Promise.all(
        workspaces.map(async workspace => {
          const threads = await workspace.threads
            .fetch()
            .then(threads =>
              threads.map(thread =>
                WorkspaceThread.toWorkspaceThreadObject(thread),
              ),
            );
  ...
  static async create({ name }: { name: string }): Promise<any> {
    ...
    let existingWorkspace = await Workspace.first([
  ...
  static async delete(
    ...
    try {
      ...
      if (!workspaces || workspaces.length === 0)
        throw new Error("No workspaces found for query");

      const workspaceSlugs: string[] = workspaces.map(
        ws => (ws as WorkspaceDBType).slug,
      );
      await database.write(async () => {
      ...
      for (const wsSlug of workspaceSlugs) {
        const threads = await WorkspaceThread.get([
          { field: "workspace_slug", value: wsSlug },
        ]);
        if (!threads || threads.length === 0) continue;
        workspaceThreadSlugs.push(
          ...threads.map(t => (t as WorkspaceThread).slug),
        );
      }

      await Promise.all(
        workspaceThreadSlugs.map(wsThreadSlug =>
          WorkspaceChat.delete([
            { field: "workspace_thread_slug", value: wsThreadSlug },
          ]),
        ),
      );
      await Promise.all(
        workspaceThreadSlugs.map(wsThreadSlug =>
          WorkspaceThread.delete([{ field: "slug", value: wsThreadSlug }]),
        ),
      );
      await Promise.all(
