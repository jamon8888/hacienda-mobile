...
export default class WorkspaceChat extends Model {
  ...
  ): Promise<WorkspaceChatType[]> {
    const chats = await database
      .get(WorkspaceChat.table)
      .query(
      ...
      .fetch();
    return chats.map(
      chat => this.toWorkspaceChatObject(chat) as WorkspaceChatType,
    );
  }
  ...
  static async create(
    data: Partial<WorkspaceChatType>,
  ): Promise<WorkspaceChatType> {
    const { uuid, workspaceThreadSlug, prompt, response } = data;

    let newWorkspaceChat: any;
    await database.write(async () => {
      newWorkspaceChat = await database
        .get(WorkspaceChat.table)
        .create((workspaceChat: any) => {
   ...
   */
  static newChatItem(data: {
    workspaceThreadSlug: string;
    prompt: string;
  }): Partial<DynamicChatMessage> & { workspaceThreadSlug: string } {
    if (!data.workspaceThreadSlug)
      throw new Error("Workspace thread slug is required");
    ...
    await database.write(async () => {
      newWorkspaceChat = await database
        ...
        .create((workspaceChat: any) => {
          ...
          if (!workspaceChat.workspaceThreadSlug)
            workspaceChat.workspaceThreadSlug = data.workspaceThreadSlug;
