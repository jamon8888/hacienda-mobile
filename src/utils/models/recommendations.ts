...
import { CACTUS_VOICE_MODELS, CactusVoiceModelId } from '@/utils/models/defaults';
...
function evaluateModel(model: any, device: DeviceCapabilities, ramGB: number, hasNPU: boolean): ModelRecommendation {
  ...
  const isCactusModel = modelId.includes('gemma-4-e2b') || modelId.includes('parakeet');
  
  // Determine recommendation
  let isRecommended = false;
  let reason = '';
  let qualityScore = 50;
  let deviceTier: 'optimal' | 'good' | 'acceptable' | 'not_recommended' = 'acceptable';
  ...
  if (modelType === 'llm') {
    ...
    if (isCactusModel) {
      ...
      if (ramGB >= 5) {
        isRecommended = true;
        reason = `Optimal: ${ramGB.toFixed(1)}GB RAM + ${hasNPU ? device.npuBackend : 'CPU'} → CQ4 best quality`;
    ...
    if (isCactusModel && modelId.includes('parakeet')) {
      if (ramGB >= 3) {
        isRecommended = true;
        reason = `Optimal: ${ramGB.toFixed(1)}GB RAM → CQ4 best accuracy (WER 0.115)`;
        qualityScore = 95;
        deviceTier = 'optimal';
      } else if (ramGB >= 2) {
        isRecommended = true;
        reason = `Good: ${ramGB.toFixed(1)}GB RAM → CQ2 minimal footprint (WER 0.147)`;
        qualityScore = 85;
        deviceTier = 'good';
      } else {
...
export function getRecommendedQuantization(
  ...
  if (modelType === 'llm') {
    if (ramGB >= 5) return { quantization: 'CQ4', reason: `High RAM (${ramGB.toFixed(1)}GB) + ${hasNPU ? device.npuBackend : 'CPU'} → best quality` };
    if (ramGB >= 3) return { quantization: 'CQ2.54', reason: `Medium RAM (${ramGB.toFixed(1)}GB) → production bundle` };
    return { quantization: 'CQ2.54', reason: `Low RAM (${ramGB.toFixed(1)}GB) → minimum viable` };
  }
  
  if (modelType === 'asr') {
    if (ramGB >= 3) return { quantization: 'CQ4', reason: `High RAM (${ramGB.toFixed(1)}GB) → best accuracy (WER 0.115)` };
    if (ramGB >= 2) return { quantization: 'CQ2', reason: `Medium RAM (${ramGB.toFixed(1)}GB) → minimal footprint (WER 0.147)` };
    return { quantization: 'CQ2', reason: `Low RAM (${ramGB.toFixed(1)}GB) → minimal footprint` };
  }
  
  if (modelType === 'embedding') {
    if (ramGB >= 3) return { quantization: 'Q4', reason: 'High RAM → best embedding quality' };
    return { quantization: 'Q4', reason: 'Standard quantization for embeddings' };
  }
