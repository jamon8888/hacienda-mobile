...
export function getHFDefaultSettings(hfModel: HuggingFaceModel): {
  ...
  const _defaultCompletionParams = {
    ...
    stopSequences: _defaultChatTemplate.eosToken
      ? [_defaultChatTemplate.eosToken]
      : [],
  };
 ...
 */
export function parseThinkingParts(text: string): {
  nonThinkingText: string;
  thinkingText: string;
  ...
  let thinkingText = "";
  let isCompleteThought = ["</think>", "</thought>", "</thinking>"].some(tag =>
    text.includes(tag),
  );

  if (!isCompleteThought) {
    thinkingText =
      text.match(/<think>([\s\S]*)/)?.[1] ||
  ...
  nonThinkingText = nonThinkingText.replace(
  ...
  return {
    nonThinkingText: nonThinkingText.trim(),
