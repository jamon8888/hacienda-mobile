 ...
 */
export async function storeProcessedFileAsText(
  filename: string,
  content: string,
) {
  if (!(await RNFS.exists(PROCESSED_FOLDER_PATH)))
    await RNFS.mkdir(PROCESSED_FOLDER_PATH);
 ...
 */
export async function searchProcessedFilesFor(
  filename: string,
  searchMethod: FsSearchMethod = "eq",
): Promise<string | null> {
  ...
  if (searchMethod === "fuzzy") {
    const availableFileNames = availableFiles.map(file =>
      file.name.toLowerCase(),
    );
    const closestFile = Levenshtein.closest(
      filename.toLowerCase(),
      availableFileNames,
    );
    if (
      closestFile &&
      Levenshtein.distance(filename.toLowerCase(), closestFile) <= 2
    )
      foundFile =
        availableFiles.find(file => file.name.toLowerCase() === closestFile) ??
        null;
  } else {
    for (const file of availableFiles) {
      ...
      if (
        ...
        foundFile = file;
      if (searchMethod === "regex" && new RegExp(filename).test(file.name))
        foundFile = file;
      if (foundFile) break;
    }
  }
