...
describe('embed', () => {
  it('constructs CactusLM from the local model path, initializes it, and embeds with normalize disabled', async () => {
    ...
    expect(mockCactusLM).toHaveBeenCalledWith({ model: '/mock/models/embed.gguf' });
    expect(mockLmInstance.init).toHaveBeenCalled();
    expect(mockLmInstance.embed).toHaveBeenCalledWith({ text: 'search_query: hello world', normalize: false });
...
describe('embedBatch', () => {
  it('embeds every text with the document prefix', async () => {
    ...
    expect(mockLmInstance.embed).toHaveBeenNthCalledWith(1, { text: 'search_document: a', normalize: false });
    expect(mockLmInstance.embed).toHaveBeenNthCalledWith(2, { text: 'search_document: b', normalize: false });
  });
});
