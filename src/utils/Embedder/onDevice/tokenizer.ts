...
export function tokenize(text: string, maxLength = TOKENIZER_CONSTANTS.MAX_LENGTH): number[] {
  if (!spmProcessor) {
    throw new Error('Tokenizer not initialized. Call initializeTokenizer() first.');
  }

  const ids = spmProcessor.encodeIds(text);
  const tokens: number[] = [TOKENIZER_CONSTANTS.CLS_TOKEN, ...ids, TOKENIZER_CONSTANTS.SEP_TOKEN];
...
export function decode(tokens: number[]): string {
  ...
  const filteredTokens = tokens.filter(t => t >= TOKENIZER_CONSTANTS.MASK_TOKEN);
