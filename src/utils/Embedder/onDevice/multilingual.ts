...
const MODEL_PREFIXES: Record<
  MultilingualEmbeddingModelId,
  Record<EmbedderPrefixType, string>
> = {
...
export default class MultilingualEmbedderProvider implements EmbeddingProvider {
  ...
  static getInstance(
    modelId: MultilingualEmbeddingModelId,
  ): MultilingualEmbedderProvider {
    if (!MultilingualEmbedderProvider.instances.has(modelId)) {
  ...
  private log(text: string, ...args: any[]) {
    console.log(
      `\x1b[35m[MultilingualEmbedder:${this.modelId}]\x1b[0m ${text}`,
      ...args,
    );
  }
  ...
  private async doInitialize(): Promise<boolean> {
    try {
      ...
      this.log(
        `Initialized successfully with context length ${this.modelConfig.contextLength}`,
      );
      return true;
    } catch (error) {
  ...
  async embed(
    ...
    return this.wrapInKeepAlive(async () => {
      ...
      if (!this.cactusLmContext)
        throw new Error(
          "MultilingualEmbedderProvider::embed: could not initialize",
        );
  ...
  async embedBatch(
    ...
    return this.wrapInKeepAlive(async () => {
      ...
      if (!this.cactusLmContext)
        throw new Error(
          "MultilingualEmbedderProvider::embedBatch: could not initialize",
        );
  ...
  async splitAndEmbed(
    ...
    const textSplitter = new TextSplitter({
      ...
      chunkSize: Math.min(
        ...
        this.modelConfig.contextLength - 50,
      ),
    ...
    this.log(
      `Split document into ${chunks.length} chunks (max ${this.modelConfig.contextLength} tokens)`,
    );
