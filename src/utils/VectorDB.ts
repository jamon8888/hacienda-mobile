...
interface VectorBoxInterface {
  ...
  bulkInsert(
    workspaceSlug: string,
    embeddings: { embedding: number[]; metadata: string }[],
  ): Promise<number[]>;
  ...
  semanticSearch(
    ...
    queryVector: number[],
    topN: number,
  ): Promise<{ id: number; metadata: string; score: number }[]>;
...
class VectorDB {
  ...
  async insertVector(
    ...
    try {
      return await this.vectorBox.insert(
        workspaceSlug,
        embedding,
  ...
  async bulkInsert(
    workspaceSlug: string,
    vectors: { embedding: number[]; metadata: object }[],
    ...
    try {
      ...
      if (
        !vectors.every(
          vector =>
            vector.embedding && vector.embedding.length > 0 && vector.metadata,
        )
      )
      ...
      const preparedVectors = vectors.map(vector => ({
        ...
        metadata:
          typeof vector.metadata === "string"
            ? vector.metadata
            : JSON.stringify(vector.metadata),
      }));

      const result = await this.vectorBox.bulkInsert(
        workspaceSlug,
        preparedVectors,
      );
  ...
  async runSemanticSearch(
    ...
    try {
      const results = await this.vectorBox.semanticSearch(
        workspaceSlug,
        queryVector,
        topN,
      );
      return results.map(
        (result: { id: number; metadata: string; score: number }) => ({
          id: result.id,
