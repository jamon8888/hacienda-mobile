...
class OllamaProvider extends BaseOpenAILikeProvider {
  ...
  async getModelDetails({
    ...
    try {
      ...
      const details: null | { capabilities: string[] } = await fetch(
        `${apiBaseUrl}/api/show`,
        {
          method: "POST",
        ...
        .then(res => {
          if (!res.ok)
            throw new Error(
              `Failed to fetch model details for ${modelTag}. Assuming no tool support.`,
            );
          return res.json();
        })
        .catch(error => {
  ...
  private async checkToolSupport(): Promise<boolean> {
    ...
    try {
      const modelTagDetails = await this.getModelDetails({
        baseUrl: this.baseURL,
        modelTag: this.model,
      });
      if (!modelTagDetails)
        throw new Error("Could not fetch tag details for model");
      const hasToolSupport =
        modelTagDetails?.capabilities?.includes("tools") || false;
   ...
   */
  override async streamGetChatCompletion(
    messages: any[] = [],
    availableTools: any[] = [],
    ...
    if (availableTools.length > 0 && !(await this.checkToolSupport())) {
      this.log(
        `Model ${this.model} does not support tools, removing them from request.`,
      );
      tools = [];
    }
