...
jest.mock('../Telemetry', () => ({
  __esModule: true,
  default: { logEvent: jest.fn(), CUSTOM_EVENTS: { ACTIONS: { TOOL_CALLED: 'tool_called' } } },
}));
...
function generateSignature(name: string, argumentsJson: string): string {
  return (ToolsManager as any)._generateToolCallSignature({ function: { name, arguments: argumentsJson } });
}

describe('_generateToolCallSignature', () => {
  it('formats a JSON-string-arguments tool call into a readable signature', () => {
    expect(generateSignature('get_weather', JSON.stringify({ city: 'Paris', unit: 'celsius' })))
      .toBe('get_weather(city: Paris, unit: celsius)');
  });

  it('renders a no-argument call as name()', () => {
    expect(generateSignature('get_current_time', '{}')).toBe('get_current_time()');
  });

  it('falls back to a raw string rather than throwing on malformed JSON', () => {
    expect(generateSignature('broken_tool', 'not json')).toBe('broken_tool("not json")');
  });
