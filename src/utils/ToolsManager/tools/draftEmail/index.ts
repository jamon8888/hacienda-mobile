...
export default {
  ...
  definition: {
    ...
    function: {
      name: "draft_email",
      description:
        "Use the assistant to draft and email for you. Includes the subject, body, and optional recipient.",
      parameters: {
        type: "object",
        properties: {
          subject: {
  ...
  config: {},
  execute: (
    args: { subject: string; body: string; to?: string },
    streamEmitter: (event: IStreamEvent, data: any) => void,
  ) => {
    try {
      const { subject, body, to } =
        typeof args === "string" ? safeJsonParse(args) : args;
      if (!subject || !body)
        return `No subject or body provided. No email was drafted.`;
      streamEmitter("report_action", {
        type: "email",
        action: {
          title: "Open Draft Email",
          link: `mailto:${to ?? ""}?subject=${encodeURIComponent(
            subject,
