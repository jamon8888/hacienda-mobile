...
export default {
  ...
  definition: {
    ...
    function: {
      name: "summarize",
      description:
        "Summarize content from a given URL or filename. Returns the summary of the content.",
      parameters: {
        type: "object",
        properties: {
          type: {
          ...
          input: {
            type: "string",
            description:
              "The URL of the website to summarize or the name of the file to summarize.",
          },
        },
        required: ["type", "input"],
      },
  ...
  execute: async function (
    ...
    try {
      const { type, input } = safeJsonParse(args, {
        type: "unknown",
        input: "",
      }) as { type: string; input: string };
      if (!type || !input) return `No input provided. No results were found.`;
      if (!this.config.validTypes.includes(type as any))
        return `Invalid type provided. Valid types are: ${this.config.validTypes.join(
          ", ",
        )}`;
      ...
      if (type === "url") {
        ...
        citation = {
          ...
          reference: {
            ...
            content:
              scrapeResult?.content?.length > 1000
                ? scrapeResult.content.substring(0, 1000) + "..."
      ...
      if (type === "filename") {
        contentToSummarize =
          (await searchProcessedFilesFor(input, "fuzzy")) ?? "";
        citation = {
          type: "document",
          document: {
            uuid: generateUUID(),
      ...
      if (!llmProvider)
        return `Error: Could not initialize LLM provider for summarization.`;
      ...
      if (limitedChunks.length !== textChunks.length)
        streamEmitter(
          "report_in_progress_thought",
          `Document was very long. Processing first ${limitedChunks.length} sections for summary.`,
        );
      const finalSummary = await this._createHierarchicalSummary(
        limitedChunks,
        llmProvider,
        streamEmitter,
      );
  ...
  _getLLMProvider: async function () {
    ...
    if (preferences.provider === "unknown")
      throw new Error("LLM provider is unknown");
  ...
  _summarizeChunk: async function (
    chunk: string,
    llmProvider: any,
  ): Promise<string> {
  ...
  _createHierarchicalSummary: async function (
    chunks: string[],
    llmProvider: any,
    streamEmitter: (event: IStreamEvent, data: any) => void,
  ): Promise<string> {
    ...
    streamEmitter(
      "report_in_progress_thought",
      "Combining summaries into final document summary...",
    );
    ...
    const finalMessages = [
      ...
      {
        role: "user",
        content: `Please create a comprehensive summary of this document based on these section summaries:\n\n${combinedSummaries}`,
      },
    ];
    try {
      const finalResponse = await llmProvider.runBasicChatCompletion(
