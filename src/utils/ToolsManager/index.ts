...
import {
  ...
  IStreamCallback,
  IStreamEvent,
} from "../AiProviders/baseOpenAILikeProvider";
...
class ToolsManager {
  ...
  async injectAvailableTools(): Promise<ToolManagerTool["definition"][]> {
    try {
      ...
      this.log(
        "ToolsManager::injectAvailableTools: Error getting available tools",
        error,
      );
      return [];
    }
  ...
  private _generateToolCallSignature(toolCall: ToolCall) {
    ...
    if (Object.keys(args).length > 0 && args !== "{}") {
      ...
      const argsString = Object.entries(parsedArgs)
  ...
  async manageToolCallExecutions(
    ...
    for (const toolCall of toolCalls) {
      ...
      const knownToolConfig = this._tools?.find(
        tool => tool.definition.function.name === toolCall.function.name,
      );
      if (!knownToolConfig) {
        this.log(
          `ToolsManager::manageToolCallExecutions: Tool not found or available: ${toolCallName}`,
        );
      ...
      this.log(
        `ToolsManager::manageToolCallExecutions: Executing tool call: ${toolCallName}`,
      );
      const toolCallResult = await knownToolConfig.execute(
      ...
      Telemetry.logEvent(Telemetry.CUSTOM_EVENTS.ACTIONS.TOOL_CALLED, {
  ...
  async toolCallLoop({
    ...
    do {
      nextMessages = await this.manageToolCallExecutions(
        nextResponse.toolCalls ?? [],
        streamEmitter,
        nextMessages,
      );
      for (const [index, message] of nextMessages.entries()) {
        if (message.role === "tool" && mergeToolCallResults) {
          ...
          nextMessages[index - 1] = {
            ...previousMessage,
            content: `${previousMessage.content}\nFunction: ${message.signature}\nResult: ${message.content}`,
          };
          availableTools = availableTools.filter(
            tool => tool.function.name !== message.function,
          ); // Remove the tool from the available tools
          nextMessages.pop(); // Remove the tool message
        }
      }

      nextResponse = await runStreamCompletion(
        nextMessages,
        (token: string) => streamEmitter("chunk", token),
        availableTools,
      );
