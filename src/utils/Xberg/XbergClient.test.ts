...
describe('extract', () => {
  ...
  it('reports malformed native JSON as an error instead of returning a string', async () => {
    ...
    await expect(XbergClient.extract('/docs/a.pdf')).rejects.toThrow(/malformed JSON/);
  });
  ...
  it('routes audio to transcription rather than the document extractors', async () => {
    nativeMock.transcribeAudio.mockResolvedValue(nativeEnvelope([docResult('spoken words')]));

    const result = await XbergClient.extract('/media/note.m4a', { transcription: { model: 'base', language: 'fr' } });

    expect(nativeMock.extract).not.toHaveBeenCalled();
    expect(nativeMock.transcribeAudio).toHaveBeenCalledWith('/media/note.m4a', 'base', 'fr');
  ...
  it('forces OCR for images when an OCR backend is configured but forceOcr is unset', async () => {
    nativeMock.extract.mockResolvedValue(nativeEnvelope([docResult('scanned')]));

    await XbergClient.extract('/photos/receipt.png', { ocr: { backend: 'tesseract' } });
  ...
  it('respects an explicit forceOcr:false on an image', async () => {
    ...
    await XbergClient.extract('/photos/receipt.png', { ocr: { backend: 'tesseract' }, forceOcr: false });
...
describe('extractBatch', () => {
  it('splits the input across several native calls and merges the results', async () => {
    nativeMock.extractBatch.mockImplementation((paths: string[]) =>
      Promise.resolve(nativeEnvelope(paths.map((p: string) => docResult(`content:${p}`, p)))),
    );
    ...
    expect(result.results.map((r: { source?: string }) => r.source)).toEqual(paths);
  });

  it('reports progress once per chunk, ending at the total', async () => {
    nativeMock.extractBatch.mockImplementation((paths: string[]) =>
      Promise.resolve(nativeEnvelope(paths.map((p: string) => docResult('x', p)))),
    );
    ...
    await XbergClient.extractBatch(
      ...
      {},
      { chunkSize: 2, onProgress: ({ done }: { done: number }) => progress.push(done) },
    );
  ...
  it('keeps successful chunks when another chunk rejects', async () => {
    nativeMock.extractBatch
      .mockResolvedValueOnce(nativeEnvelope([docResult('ok-a', '/a.pdf'), docResult('ok-b', '/b.pdf')]))
      .mockRejectedValueOnce(new Error('native blew up'));

    const result = await XbergClient.extractBatch(['/a.pdf', '/b.pdf', '/c.pdf', '/d.pdf'], {}, { chunkSize: 2 });

    expect(result.results.map((r: { source?: string }) => r.source)).toEqual(['/a.pdf', '/b.pdf']);
    expect(result.errors).toEqual([
  ...
  it('transcribes audio separately and tags each transcript with its source path', async () => {
    nativeMock.extractBatch.mockResolvedValue(nativeEnvelope([docResult('doc', '/a.pdf')]));
    // The single-file transcription path never tags a source; extractBatch must add it.
    nativeMock.transcribeAudio.mockResolvedValue(nativeEnvelope([docResult('spoken')]));

    const result = await XbergClient.extractBatch(['/a.pdf', '/voice.mp3']);

    expect(nativeMock.extractBatch).toHaveBeenCalledWith(['/a.pdf'], expect.any(String));
    const bySource = Object.fromEntries(
      result.results.map((r: { source?: string; content: string }) => [r.source, r.content]),
    );
  ...
  it('records a failed transcription as an error rather than aborting the batch', async () => {
    ...
    expect(result.results).toHaveLength(1);
    expect(result.errors).toEqual([
      { source: '/voice.mp3', error: 'no whisper model', code: 'TRANSCRIPTION_ERROR' },
    ]);
  });
