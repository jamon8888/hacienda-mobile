...
describe('Memory Schema', () => {
  ...
  it('should have valid FTS5 schema', () => {
    expect(MEMORY_FTS_SCHEMA).toContain('CREATE VIRTUAL TABLE IF NOT EXISTS memories_fts USING fts5');
    expect(MEMORY_FTS_SCHEMA).toContain('content');
    expect(MEMORY_FTS_SCHEMA).toContain('summary');
  });

  it('should have valid vec0 schema', () => {
    expect(MEMORY_VEC_SCHEMA).toContain('CREATE VIRTUAL TABLE IF NOT EXISTS memories_vec USING vec0');
