...
export default function OnDeviceLLMOptions({
  selection,
  onChange,
}: {
  selection: ISelection;
  onChange: (config: Record<string, any>, autoConfirm?: boolean) => void;
}) {
  const [selectedModel, setSelectedModel] = useState<
    Model | NPUEnabledModel | null
  ...
  function handleModelSelection(model: Model | NPUEnabledModel) {
    ...
    Alert.alert(
      "Download model?",
      `This will download the model to your device. It is ${formatBytes(
        model.size,
      )} bytes.`,
      [
        {
      ...
      <ScrollView className="h-[80vh]">
        <View className="flex-col gap-y-4">
          {defaultModels?.map(model => {
            ...
            const badgeColor =
              model.runtime === "NPU" ? "bg-purple-500/50" : "bg-green-300/50";
            return (
              <TouchableOpacity
                key={model.id}
                ...
                <Text className={`${textColor} text-xs italic`}>
                  {model.capabilities?.join(", ")} | {formatBytes(model.size)}
                </Text>
              </TouchableOpacity>
            );
          })}
