...
try {
  ...
  if (!keystorePassword) {
    console.error(
      "APP_RELEASE_STORE_PASSWORD environment variable is not set.",
    );
    process.exit(1);
  }
  if (!fs.existsSync(keystorePath)) {
    console.error(
      "Keystore file not found. Please ensure the keystore file exists at android/app/anythingllm-upload-key.keystore",
    );
    process.exit(1);
  }
  ...
  fs.copyFileSync(
    aabPath,
    path.join(releaseFolder, `anythingllm-v${version}.aab`),
  );
  ...
  fs.copyFileSync(
    path.join(outputDir, `anythingllm-universal.apk`),
    path.join(latestFolder, `anythingllm-universal.apk`),
  );
  ...
  fs.copyFileSync(
    path.join(outputDir, `anythingllm-universal.apk`),
    path.join(legacyFolder, `anythingllm-universal.apk`),
  );
  ...
  console.log(
    `📦 AAB bundle: ${path.join(releaseFolder, `anythingllm-v${version}.aab`)}`,
  );
  console.log(
    `🎯 Final APK: ${path.join(releaseFolder, `anythingllm-universal.apk`)}`,
  );
} catch (error) {
